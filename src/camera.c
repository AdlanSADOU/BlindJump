...
void init_view(all_t *s_all)
{
    ...
    sfSprite_setScale(s_all->s_effect.vignette1, (sfVector2f){1.71, 1.5});
    sfSprite_setScale(s_all->s_effect.vignette2, (sfVector2f){1.71, 1.5});
}
...
void move_camera2(all_t *s_all)
{
    ...
    && s_all->s_effect.seconds > 0.01 && s_all->s_effect.anim < 50) {
        ...
        sfRenderWindow_drawSprite(
            s_all->s_game.window,
            s_all->s_effect.vignette2,
            s_all->s_effect.vignette_state
        );
        ...
        sfRenderWindow_drawSprite(s_all->s_game.window, s_all->s_effect.vignette2, s_all->s_effect.vignette_state);
        sfRenderWindow_drawRectangleShape(s_all->s_game.window,s_all->s_tp.black, NULL);
    }
}
...
void move_camera(all_t *s_all)
{
    sfView_setCenter(s_all->s_game.camera,
        ...
        s_all->s_player.hero_pos.y + 15});
    sfRenderWindow_setView(s_all->s_game.window, s_all->s_game.camera);
    s_all->s_effect.vignette_pos = (sfVector2f){s_all->s_player.hero_pos.x - 375, s_all->s_player.hero_pos.y - 194-20};

    sfSprite_setPosition(s_all->s_effect.vignette1, s_all->s_effect.vignette_pos);
    sfSprite_setPosition(s_all->s_effect.vignette2, s_all->s_effect.vignette_pos);
    sfRectangleShape_setPosition(s_all->s_tp.black, (sfVector2f){s_all->s_player.hero_pos.x - 374, s_all->s_player.hero_pos.y - 193});

    if (s_all->s_game.scene == SPAWN)
        sfRenderWindow_drawSprite(s_all->s_game.window, s_all->s_effect.vignette1, NULL);
    move_camera2(s_all);
}